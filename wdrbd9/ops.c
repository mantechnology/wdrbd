...
IOCTL_MountVolume(PDEVICE_OBJECT DeviceObject, PIRP Irp, PULONG ReturnLength)
{
	...
	*ReturnLength = 0;
    ...
    if (!pvext->Active)
	{
		RtlStringCbPrintfA(Message, sizeof(Message), "%wZ volume is not dismounted", &pvext->MountPoint);
...
#endif
	{
		RtlStringCbPrintfA(Message, sizeof(Message), "%wZ volume is handling by drbd. Failed to mount",
			&pvext->MountPoint);
		*ReturnLength = (ULONG)strlen(Message);
		WDRBD_ERROR("%s\n", Message);
