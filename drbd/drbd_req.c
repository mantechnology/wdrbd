		...
		int clear, int set)
{
	...
	if (!(old_net & RQ_NET_DONE) && (set & RQ_NET_DONE)) {
		...
		if (atomic_read(&g_featurelog_flag) & FEATURELOG_FLAG_LATENCY) {
			...
			WDRBD_LATENCY("req(%p) NET latency : type(%s) sector(%llu) size(%u) net(%lldus)\n", 
				req, (req->rq_state[0] & RQ_WRITE) ? "write" : "read", req->i.sector, req->i.size, timestamp_elapse(req->net_sent_ts[peer_device->node_id], req->net_done_ts[peer_device->node_id]));
		}
