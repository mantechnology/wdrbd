...
static int split_recv_resync_read(struct drbd_peer_device *peer_device, struct drbd_peer_request_details *d) __releases(local)
{
					...
					if (marked_rl) {
						...
						for (int i = 0; i < sizeof(marked_rl->marked_rl) * 8; i++) {
							...
							if (!(marked_rl->marked_rl & 1 << i)) {
								...
								drbd_set_sync(device, split_peer_req->i.sector, split_peer_req->i.size, bits, DRBD_END_OF_BITMAP);
	...
	all_out_of_sync:
		...
		* respective _drbd_clear_done_ee */
		...
		drbd_set_sync(device, peer_req->i.sector, peer_req->i.size, bits, DRBD_END_OF_BITMAP);
